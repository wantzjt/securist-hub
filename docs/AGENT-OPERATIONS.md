...
## Delivery cadence (D-010)

Process exists to preserve product authority and safe delivery; it does not overrule explicit founder scope. D-010 supersedes D-009's absolute product/process freeze while retaining one-work-order discipline and human safety gates.

Do not invent competing release frameworks or management theater. A human may explicitly authorize a product or operational work order without first satisfying an agent-created gate.
...
These tracks remain important and human-owned. They are not an absolute veto over a separately authorized product work order. Product work must still preserve LIVE/SEED honesty, private-data boundaries, and the Decision Graph authority model.
...
- Carry uncommitted or untracked, non-ignored files between work orders.
...
```text
Explicitly authorized work order
...
| [`INTERNAL-SYSTEM-GRAPH.md`](./INTERNAL-SYSTEM-GRAPH.md) | Repository authority and regression map |
