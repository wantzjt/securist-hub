...
export function mapDecision(row: Record<string, unknown>): Decision {
  ...
  if (raw && typeof raw === 'object') {
    ...
    const env =
      (s.environment as DecisionScope['environment'] | undefined) ??
      'development'
    const dclass =
      (s.dataClassification as DecisionScope['dataClassification'] | undefined) ??
      (s.data_classification as DecisionScope['dataClassification'] | undefined) ??
      'public'
    const boundary =
      (s.deploymentBoundary as DecisionScope['deploymentBoundary'] | undefined) ??
      (s.deployment_boundary as DecisionScope['deploymentBoundary'] | undefined) ??
      'local_only'
    scope = {
      tenantId: String(s.tenantId ?? s.tenant_id ?? row.tenant_id),
      environment: env,
      intendedUse: String(s.intendedUse ?? s.intended_use ?? ''),
      dataClassification: dclass,
